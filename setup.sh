...
main() {
    ...
    detect_lang() {
        printf "\n"
        printf "  ${BOLD}1)${NC} Русский\n"
        printf "  ${BOLD}2)${NC} English\n"
        printf "\n"
        printf "Выберите язык / Select language [1]: "
...
XMEOW_EOF
    ...
    do_update() {
        ...
        if [ "$CUR_VER" = "$VERSION" ]; then
            success "$(msg "Сервер уже последней версии ($VERSION)" \
                          "Server already at latest version ($VERSION)")"
        else
            SERVER_UPDATED=true
            download_and_verify
        fi

        if $SERVER_UPDATED; then
            # Run stop+install+start in a detached subshell so it survives
            # if the terminal disconnects (e.g., updating via web terminal
            # kills xmeow-server which drops the WebSocket connection).
            (
                stop_service
                install_binary
                install_extui
            ...
            # Wait briefly for the background process
            sleep 3

            # If still running, it's fine — service will start on its own
            if kill -0 "$UPGRADE_PID" 2>/dev/null; then
                info "$(msg "Обновление продолжается в фоне..." "Update continuing in background...")"
                wait "$UPGRADE_PID" 2>/dev/null || true
            fi
        fi

        # Update agent if installed (download in foreground, replace in background)
        if [ -x "$INSTALL_DIR/$AGENT_BIN" ]; then
            AGENT_CUR=$("$INSTALL_DIR/$AGENT_BIN" --version 2>/dev/null) || AGENT_CUR="?"
            if [ "$AGENT_CUR" != "$VERSION" ]; then
                info "$(msg "Обновление агента: v$AGENT_CUR → v$VERSION" "Updating agent: v$AGENT_CUR → v$VERSION")"
                AGENT_ARCHIVE="${AGENT_BIN}-${SUFFIX}.tar.gz"
                AGENT_URL="https://github.com/$REPO/releases/download/v$VERSION/$AGENT_ARCHIVE"
                mkdir -p "$TMP_DIR"
                curl --progress-bar --retry 3 -fSL -o "$TMP_DIR/$AGENT_ARCHIVE" "$AGENT_URL" 2>&1 && {
                    AGENT_INNER="${AGENT_BIN}-linux-${ARCH}"
                    tar -xzf "$TMP_DIR/$AGENT_ARCHIVE" -C "$TMP_DIR" "$AGENT_INNER" 2>/dev/null && {
                        # Stop/replace/start in background to survive terminal disconnect
                        (
                            [ -f "$AGENT_INITD" ] && "$AGENT_INITD" stop 2>/dev/null || true
                            mv "$TMP_DIR/$AGENT_INNER" "$INSTALL_DIR/$AGENT_BIN"
                            chmod +x "$INSTALL_DIR/$AGENT_BIN"
                            [ -f "$AGENT_INITD" ] && "$AGENT_INITD" start 2>/dev/null || true
                            rm -f "$TMP_DIR/$AGENT_ARCHIVE"
                        ) </dev/null >/dev/null 2>&1 &
                        wait $! 2>/dev/null || true
                        success "$(msg "Агент обновлён до v$VERSION" "Agent updated to v$VERSION")"
                    } || warn "$(msg "Не удалось распаковать агент" "Failed to extract agent")"
                } || warn "$(msg "Не удалось скачать агент" "Failed to download agent")"
            else
                info "$(msg "Агент уже v$VERSION" "Agent already v$VERSION")"
            fi
        fi
    ...
    uninstall_server() {
        msg "Удаление сервера..." "Uninstalling server..."
        ...
        # Ask about config
        if [ -f "$CONF_FILE" ]; then
            printf "${YELLOW}[?]${NC} $(msg "Удалить конфиг сервера $CONF_FILE? (y/N): " \
                                            "Remove server config $CONF_FILE? (y/N): ")"
            ...
            case "$REPLY" in
                y|Y|yes|Yes)
                    ...
                    ;;
                *)
                    info "$(msg "Конфиг сохранен" "Config kept")"
                    ;;
            esac
        fi

        success "$(msg "Сервер удален!" "Server uninstalled!")"
    }

    uninstall_agent() {
        msg "Удаление агента..." "Uninstalling agent..."

        # Stop agent
        if [ -f "$AGENT_INITD" ]; then
            "$AGENT_INITD" stop 2>/dev/null || true
        elif [ -f "$AGENT_PIDFILE" ]; then
            kill "$(cat "$AGENT_PIDFILE")" 2>/dev/null || true
            rm -f "$AGENT_PIDFILE"
        fi
        ...
        if [ -f "$INSTALL_DIR/$AGENT_BIN" ]; then
            ...
            success "$(msg "Бинарник агента удален" "Agent binary removed")"
        fi

        # Remove agent init.d script
        if [ -f "$AGENT_INITD" ]; then
            rm -f "$AGENT_INITD"
            success "$(msg "Сервис агента удален" "Agent service removed")"
        fi

        # Remove agent PID file
        rm -f "$AGENT_PIDFILE"

        # Remove xmeow command (only if server is also not installed)
        if [ -f "$XMEOW_CMD" ] && [ ! -x "$INITD_SCRIPT" ]; then
            rm -f "$XMEOW_CMD"
            success "$(msg "Команда xmeow удалена" "Command xmeow removed")"
        elif [ -f "$XMEOW_CMD" ]; then
            info "$(msg "Команда xmeow сохранена (сервер ещё установлен)" \
                        "Command xmeow kept (server still installed)")"
        fi

        # Ask about agent config
        if [ -f "$AGENT_CONF" ]; then
            printf "${YELLOW}[?]${NC} $(msg "Удалить конфиг агента $AGENT_CONF? (y/N): " \
                                            "Remove agent config $AGENT_CONF? (y/N): ")"
            read -r REPLY < /dev/tty 2>/dev/null || REPLY="n"
            case "$REPLY" in
                y|Y|yes|Yes)
                    rm -f "$AGENT_CONF"
                    success "$(msg "Конфиг агента удален" "Agent config removed")"
                    ;;
                *)
                    info "$(msg "Конфиг сохранен" "Config kept")"
                    ;;
        ...
        success "$(msg "Агент удален!" "Agent uninstalled!")"
    }

    do_uninstall() {
        HAS_SERVER=false
        HAS_AGENT=false
        [ -f "$INSTALL_DIR/$BIN_NAME" ] || [ -f "$INITD_SCRIPT" ] && HAS_SERVER=true
        [ -f "$INSTALL_DIR/$AGENT_BIN" ] || [ -f "$AGENT_INITD" ] && HAS_AGENT=true

        if ! $HAS_SERVER && ! $HAS_AGENT; then
            info "$(msg "Ничего не установлено" "Nothing installed")"
            return 0
        fi

        printf "\n"
        printf "  $(msg "Что удалить?" "What to uninstall?")\n\n"

        if $HAS_SERVER && $HAS_AGENT; then
            printf "  ${BOLD}1)${NC} $(msg "Сервер" "Server")      $(msg "— xmeow-server + дашборд" "— xmeow-server + dashboard")\n"
            printf "  ${BOLD}2)${NC} $(msg "Агент" "Agent")        $(msg "— xmeow-agent" "— xmeow-agent")\n"
            printf "  ${BOLD}3)${NC} $(msg "Всё" "Everything")     $(msg "— сервер + агент" "— server + agent")\n"
            printf "  ${BOLD}0)${NC} $(msg "Отмена" "Cancel")\n"
            printf "\n"
            printf "$(msg "Выберите" "Select") [0-3]: "
            read -r CHOICE < /dev/tty 2>/dev/null || CHOICE="0"
            case "$CHOICE" in
                1) uninstall_server ;;
                2) uninstall_agent ;;
                3) uninstall_server; uninstall_agent ;;
                0) msg "Отмена." "Cancelled."; return 0 ;;
                *) die "$(msg "Неверный выбор" "Invalid choice")" ;;
            esac
        ...
        printf "\n${GREEN}${BOLD}"
        printf "====================================\n"
        msg " XMeow $(msg "удалён!" "uninstalled!")"
        printf "====================================\n"
        printf "${NC}\n"
    }
    ...
    #   INTERACTIVE MENU
    # =====================
    # --- Submenu helper: choose server/agent/both ---
    choose_target() {
        # $1 = action label (for display)
        printf "\n"
        if $HAS_SERVER && $HAS_AGENT_INSTALLED; then
            printf "  ${BOLD}1)${NC} $(msg "Сервер" "Server")\n"
            printf "  ${BOLD}2)${NC} $(msg "Агент" "Agent")\n"
            printf "  ${BOLD}3)${NC} $(msg "Всё" "Everything")\n"
            printf "  ${BOLD}0)${NC} $(msg "Назад" "Back")\n"
            printf "\n$(msg "Выберите" "Select") [0-3]: "
            read -r SUB < /dev/tty 2>/dev/null || SUB="0"
            printf '%s' "$SUB"
        elif $HAS_SERVER; then
            printf '%s' "1"
        elif $HAS_AGENT_INSTALLED; then
            printf '%s' "2"
        else
            printf '%s' "0"
        fi
    ...
    show_menu() {
        ...
        HAS_SERVER=false
        HAS_AGENT_INSTALLED=false
        AGENT_VER=""
        [ -n "$CUR_VER" ] && HAS_SERVER=true
        if [ -x "$INSTALL_DIR/$AGENT_BIN" ]; then
            HAS_AGENT_INSTALLED=true
        ...
        LATEST_TAG=$(curl -fsSL --connect-timeout 3 "https://api.github.com/repos/$REPO/releases/latest" 2>/dev/null \
            | grep '"tag_name"' | head -1 | sed 's/.*"tag_name" *: *"\([^"]*\)".*/\1/') || true
        if [ -n "$LATEST_TAG" ]; then
            LATEST_VER=$(printf '%s' "$LATEST_TAG" | sed 's/^v//')
            $HAS_SERVER && [ "$CUR_VER" != "$LATEST_VER" ] && SERVER_OUTDATED=true
            $HAS_AGENT_INSTALLED && [ "$AGENT_VER" != "$LATEST_VER" ] && AGENT_OUTDATED=true
        fi

        # --- Status ---
        printf "\n"
        if $HAS_SERVER; then
            if $SERVER_OUTDATED; then
                printf "  $(msg "Сервер" "Server"):  ${YELLOW}v$CUR_VER${NC} → ${GREEN}v$LATEST_VER${NC}\n"
            else
                printf "  $(msg "Сервер" "Server"):  ${GREEN}v$CUR_VER ✓${NC}\n"
            fi
        else
            printf "  $(msg "Сервер" "Server"):  ${RED}$(msg "не установлен" "not installed")${NC}\n"
        fi
        if $HAS_AGENT_INSTALLED; then
            if $AGENT_OUTDATED; then
                printf "  $(msg "Агент" "Agent"):   ${YELLOW}v$AGENT_VER${NC} → ${GREEN}v$LATEST_VER${NC}\n"
            else
                printf "  $(msg "Агент" "Agent"):   ${GREEN}v$AGENT_VER ✓${NC}\n"
            fi
        else
            printf "  $(msg "Агент" "Agent"):   $(msg "не установлен" "not installed")\n"
        fi
        printf "\n"
        ...
        # Install: show only if something is NOT installed
        MENU_INSTALL=""
        if ! $HAS_SERVER || ! $HAS_AGENT_INSTALLED; then
            N=$((N+1)); MENU_INSTALL=$N
            if ! $HAS_SERVER && ! $HAS_AGENT_INSTALLED; then
                printf "  ${BOLD}$N)${NC} $(msg "Установить" "Install")           $(msg "— сервер и/или агент" "— server and/or agent")\n"
            elif ! $HAS_SERVER; then
                printf "  ${BOLD}$N)${NC} $(msg "Установить сервер" "Install server")    $(msg "— сервер + дашборд" "— server + dashboard")\n"
            else
                printf "  ${BOLD}$N)${NC} $(msg "Установить агент" "Install agent")     $(msg "— для удалённого управления" "— for remote management")\n"
            fi
        fi

        # Update: show only if something is outdated
        MENU_UPDATE=""
        if $SERVER_OUTDATED || $AGENT_OUTDATED; then
            N=$((N+1)); MENU_UPDATE=$N
            UPDINFO=""
            if $SERVER_OUTDATED && $AGENT_OUTDATED; then
                UPDINFO="$(msg "сервер + агент" "server + agent") → v$LATEST_VER"
            elif $SERVER_OUTDATED; then
                UPDINFO="$(msg "сервер" "server") v$CUR_VER → v$LATEST_VER"
            else
                UPDINFO="$(msg "агент" "agent") v$AGENT_VER → v$LATEST_VER"
            fi
            printf "  ${BOLD}$N)${NC} ${GREEN}$(msg "Обновить" "Update")${NC}              $(msg "—" "—") $UPDINFO\n"
        fi

        # Reinstall: show only if something is installed
        MENU_REINSTALL=""
        if $HAS_SERVER || $HAS_AGENT_INSTALLED; then
            N=$((N+1)); MENU_REINSTALL=$N
            printf "  ${BOLD}$N)${NC} $(msg "Переустановить" "Reinstall")        $(msg "— удалить и установить заново" "— remove and install fresh")\n"
        fi

        # Uninstall: show only if something is installed
        MENU_UNINSTALL=""
        if $HAS_SERVER || $HAS_AGENT_INSTALLED; then
            N=$((N+1)); MENU_UNINSTALL=$N
            printf "  ${BOLD}$N)${NC} $(msg "Удалить" "Uninstall")              $(msg "— полное удаление" "— complete removal")\n"
        fi

        # Language switch (always)
        N=$((N+1)); MENU_LANG=$N
        LANG_LABEL="EN"; [ "$LANG_CODE" = "ru" ] && LANG_LABEL="RU"
        printf "  ${BOLD}$N)${NC} $(msg "Сменить язык" "Change language")     $(msg "— сейчас:" "— current:") $LANG_LABEL\n"
        printf "  ${BOLD}0)${NC} $(msg "Выход" "Exit")\n"
        printf "\n"

        printf "$(msg "Выберите действие" "Select action") [0-$N]: "
        read -r CHOICE < /dev/tty 2>/dev/null || CHOICE="0"

        # --- Route ---
        if [ "$CHOICE" = "0" ]; then
            msg "Выход." "Bye."; exit 0
        elif [ "$CHOICE" = "$MENU_LANG" ]; then
            detect_lang; show_menu; return
        elif [ -n "$MENU_INSTALL" ] && [ "$CHOICE" = "$MENU_INSTALL" ]; then
            if ! $HAS_SERVER && ! $HAS_AGENT_INSTALLED; then
                printf "\n"
                printf "  ${BOLD}1)${NC} $(msg "Сервер + дашборд" "Server + dashboard")\n"
                printf "  ${BOLD}2)${NC} $(msg "Агент" "Agent")\n"
                printf "  ${BOLD}3)${NC} $(msg "Всё" "Everything")\n"
                printf "  ${BOLD}0)${NC} $(msg "Назад" "Back")\n"
                printf "\n$(msg "Выберите" "Select") [0-3]: "
        ...
        elif [ -n "$MENU_UPDATE" ] && [ "$CHOICE" = "$MENU_UPDATE" ]; then
            do_update
        elif [ -n "$MENU_REINSTALL" ] && [ "$CHOICE" = "$MENU_REINSTALL" ]; then
            if $HAS_SERVER && $HAS_AGENT_INSTALLED; then
                printf "\n"
                printf "  ${BOLD}1)${NC} $(msg "Сервер" "Server")\n"
                printf "  ${BOLD}2)${NC} $(msg "Агент" "Agent")\n"
        ...
        elif [ -n "$MENU_UNINSTALL" ] && [ "$CHOICE" = "$MENU_UNINSTALL" ]; then
            do_uninstall
        else
            warn "$(msg "Неверный выбор: $CHOICE" "Invalid choice: $CHOICE")"
            show_menu; return
        fi
    }
